...
export class AddIncidentsModule1778901587849 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS \`photos\` (\`id\` varchar(36) NOT NULL, \`url_imagen\` varchar(255) NOT NULL, \`fecha_captura\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`incidentBusId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS \`incident_buses\` (\`id\` varchar(36) NOT NULL, \`nivel_gravedad\` int NULL, \`incident_id\` varchar(36) NULL, \`bus_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS \`incidents\` (\`id\` varchar(36) NOT NULL, \`titulo\` varchar(255) NOT NULL, \`descripcion\` text NOT NULL, \`categoria\` enum ('MECANICO', 'ACCIDENTE', 'CONGESTION', 'PASAJERO', 'OTRO') NOT NULL, \`estado\` enum ('REPORTADO', 'EN_REVISION', 'RESUELTO') NOT NULL DEFAULT 'REPORTADO', \`fecha_reporte\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`shiftId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS \`companies\` (\`id\` varchar(36) NOT NULL, \`nombre\` varchar(255) NOT NULL, \`nit\` varchar(255) NULL, \`telefono\` varchar(255) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS \`gps_devices\` (\`id\` varchar(36) NOT NULL, \`codigo_serial\` varchar(255) NOT NULL, \`ultima_latitud\` decimal(10,8) NULL, \`ultima_longitud\` decimal(11,8) NULL, \`ultima_actualizacion\` timestamp NULL, \`busId\` int NULL, UNIQUE INDEX \`REL_770dce13e2d3289769b3906811\` (\`busId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
